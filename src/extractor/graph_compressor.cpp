                               ...
                               CompressedEdgeContainer &geometry_compressor)
{
                ...
                rev_edge_data1.CanCombineWith(rev_edge_data2))
            {
                ...
                */
                ...
                if (node_weight_penalty && node_duration_penalty)
                {
                    ...
                    std::cout << "Weight: " << forward_weight1 << " " << forward_weight2 << " "
                              << *node_weight_penalty << std::endl;
                    std::cout << "Duration: " << forward_duration1 << " " << forward_duration2
                              << " " << *node_duration_penalty << std::endl;
                }
                ...
                // named constant
                const constexpr auto FORWARD = true;
    ...
    {
        std::cout << "Node: " << node_u << std::endl;
        for (const auto edge_id : util::irange(graph.BeginEdges(node_u), graph.EndEdges(node_u)))
        {
            std::cout << "  Edge; " << edge_id << std::endl;
            if (geometry_compressor.HasEntryForID(edge_id))
            {
                const auto &forward_bucket = geometry_compressor.GetBucketReference(edge_id);
                for (auto entry : forward_bucket)
                {
                    std::cout << "\t" << entry.node_id << " " << entry.weight << " "
